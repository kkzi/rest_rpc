...
template<size_t... idx>
auto make_tuple_from_sequence(std::index_sequence<idx...>)->decltype(std::make_tuple(idx...))
{
    std::make_tuple(idx...);
}
...
void tuple_switch(const std::size_t i, Tuple&& t, F&& f, std::index_sequence<idx...>)
{
    (void)std::initializer_list<int> { (i == idx && ((void)std::forward<F>(f)(std::integral_constant<size_t, idx>{}), 0))...  };
}
} // namespace detail
